...
import kvkSearchHandler from './api/kvk/search'
import kvkProfileHandler from './api/kvk/profile'

const createDevApiPlugin = (
  route: string,
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : `Onbekende fout in ${route} middleware`
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      react(),
      createDevApiPlugin('/api/extract', extractHandler),
